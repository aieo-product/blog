...
このアプリはmacOSのKeychainにAPIキーを暗号化して保存します。
Proxyモードでは.envに一切キーが露出しない仕組みになっているので、通常の使い方では安全性はかなり高いと思っています。

ただし、**どんなソフトウェアも100%の安全を保証できるものではありません**。
万一のキー漏洩や不具合が発生した場合も、**すべて自己責任**でお使いください。

私はできる限り丁寧に作っていますが、最終的にはご自身の判断と責任でご利用いただければ幸いです。
